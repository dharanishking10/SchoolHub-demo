...
import { LayoutDashboard, User, ClipboardCheck, BookOpen, FileText, Calendar, Mail, LogOut, Menu, X, ChevronRight, Megaphone, School, ClipboardList } from 'lucide-react'
...
const NAV = [
  ...
  { to: '/dashboard/student/results', icon: ClipboardList, label: 'Exam Results' },
      ...
      <div className="px-5 py-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-secondary text-xs font-medium mt-0.5">Student Portal</p>
          </div>
        </div>
      </div>
      <div className="px-5 py-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-secondary/20 border border-secondary/40 flex items-center justify-center">
            <span className="text-secondary font-bold text-sm">{(user?.name || 'S')[0]}</span>
          </div>
