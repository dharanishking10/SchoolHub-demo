...
interface Announcement { id: number; title: string; message: string; audience: string; className?: string | null; section?: string | null; createdById: number; createdByName: string; createdByRole: string; createdAt: string }
...
export default function TAnnouncements() {
  ...
  const submit = async () => {
    ...
    const res = await fetch('/api/announcements', {
      ...
      body: JSON.stringify({ title: title.trim(), message: message.trim(), audience: 'CLASS', className, section }),
    })
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">School-wide notices and messages you can send to your class</p>
        </div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          const canDelete = a.createdById === user?.id && a.createdByRole === 'TEACHER'
          return (
            <motion.div key={a.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {canDelete && <button onClick={() => remove(a.id)} className="text-gray-300 hover:text-red-500 shrink-0"><Trash2 size={15} /></button>}
                </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-[#0B2447]">Notify My Class</h3>
