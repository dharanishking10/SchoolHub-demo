...
import {
  ...
  ClipboardCheck, Megaphone, Download, History, Gift, Calendar, TrendingUp, ClipboardList
} from 'lucide-react'
...
const NAV = [
  ...
  { to: '/dashboard/headmaster/promotion', icon: TrendingUp, label: 'Promotion' },
  { to: '/dashboard/headmaster/examinations', icon: ClipboardList, label: 'Examinations' },
  ...
  { to: '/dashboard/headmaster/schemes', icon: Gift, label: 'Government Schemes' },
      ...
      <div className="px-5 py-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-secondary text-xs font-medium mt-0.5">Headmaster Portal</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 py-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-secondary/20 border border-secondary/40 flex items-center justify-center">
            <span className="text-secondary font-bold text-sm">{(user?.name || user?.username || 'H')[0]}</span>
          </div>
          <div className="min-w-0">
            <p className="text-white text-sm font-semibold truncate">{user?.name || user?.username}</p>
            <p className="text-white/50 text-xs">Headmaster</p>
          </div>
        </div>
      </div>
