...
const HEADMASTER_ONLY = [verifyToken, requireRole('HEADMASTER')]
...
router.get('/', verifyToken, requireRole('HEADMASTER'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { schemeName, className, section, academicYear, status, search } = req.query as Record<string, string>
    const where: Record<string, unknown> = {}
    if (schemeName) where.schemeName = schemeName
...
router.post('/', ...HEADMASTER_ONLY, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.put('/:id', ...HEADMASTER_ONLY, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.delete('/:id', ...HEADMASTER_ONLY, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const id = Number(req.params.id)
...
router.get('/export', ...HEADMASTER_ONLY, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const schemes = await prisma.governmentScheme.findMany({
      include: { student: { select: { fullName: true, rollNumber: true } } },
      orderBy: { createdAt: 'desc' },
